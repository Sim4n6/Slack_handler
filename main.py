...
def print_partition_table(partition_table):
    ...
    for partition in partition_table:
        ...
        print(f"{partition.addr}, {partition.desc}, {partition.start}s({partition.start*512}) {partition.len}", flush=True)
    print()
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        description="Handle the file slack spaces.")
    ...
    parser.add_argument("-t", "--type", default="raw",
    ...
    if arguments.csv is not None:
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            ...
            writer.writeheader()
            for s in all_slacks:
                writer.writerow({'slack filename': s.get_s_name(), 'slack size': s.get_s_size(
                ), 'partition address': s.get_s_partition_addr(), 'MD5': s.get_s_md5(), 'SHA1': s.get_s_sha1(), 'parent dirs': s.get_s_dirs()})
