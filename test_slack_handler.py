...
def test__cli_unfound_disk_img():
    proc = subprocess.Popen(["python", "main.py", "unfound_disk.img"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def test__cli_print_partition_table():
    proc = subprocess.Popen(["python", "main.py", "test_data/disk_img__scenario1_1__100_files.raw"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdoutput, error = proc.communicate()
...
def test__cli_csv_file():
    completedProcess = subprocess.Popen(["python", "main.py", "-c", "results0.csv", "test_data/di1.raw"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
